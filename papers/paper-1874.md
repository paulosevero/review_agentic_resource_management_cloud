...
- **evidence_trail:**
  ...
  - `{ category: llm_as_workload, pattern_id: wl_inference_llm_a, matched_substring: "LLM Systems

While deploying" }`
  - `{ category: llm_as_workload, pattern_id: wl_inference_llm_b, matched_substring: "deploying large language model" }`
  - `{ category: llm_as_workload, pattern_id: wl_abs_serving_llms, matched_substring: "deploying large language model" }`
...
_(populated by `/screen --stage full-text`. PDF location: `raw/pdfs/paper-1874.pdf`)_
