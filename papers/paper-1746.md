---
id: paper-1746
title: Large Language Model-Guided SARSA Algorithm for Dynamic Task Scheduling in
  Cloud Computing
authors:
...
screening:
  ...
  06-full-text-screening:
    ...
    my_justification: "LLM gera heurísticas para Q-shaping no agente SARSA, que é o decisor final de scheduling. LLM subordinado ao RL — exclui per Gate B."
    agrees_with_regex: true
    divergence_reason: null
...
- **overrides_applied:** []
- **evidence_trail:**
  - `{ category: H_llm_supports_other_method, pattern_id: llm_explicit_support_role, matched_substring: "LLM guides SARSA" }`
  - `{ category: H_llm_supports_other_method, pattern_id: llm_explicit_support_role, matched_substring: "LLM guides SARSA" }`
  - `{ category: H_llm_supports_other_method, pattern_id: llm_explicit_support_role, matched_substring: "LLM Guide RL" }`
  - `{ category: H_llm_supports_other_method, pattern_id: llm_as_feature_extractor, matched_substring: "LLM-empowered state representation" }`
