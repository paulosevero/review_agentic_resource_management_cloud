---
id: paper-2556
title: 'LLM-Driven Adaptive Cloud Resource Scheduling: Bridging Reasoning Intelligence
...
screening:
  ...
  06-full-text-screening:
    ...
    my_justification: 'LLMSched posiciona explicitamente o LLM como ''policy generator''
      que delega ''constraint enforcement and fine-grained optimization to algorithmic
      modules''. Três estágios: (1) codificação de estado em texto, (2) LLM gera candidatos
      via prompting, (3) ILP refina/otimiza. O loop autônomo que dirige scheduling
      é ILP + heuristics clássicos; LLM não é o agente agentic que percebe, raciocina,
      age. Seção IV methodology (A-E) detalha ILP como refinement obrigatório, não
      LLM como decisor.'
    agrees_with_regex: true
    divergence_reason: null
...
- **my_final_decision:** Exclude
- **my_justification:** LLMSched posiciona explicitamente o LLM como 'policy generator' que delega 'constraint enforcement and fine-grained optimization to algorithmic modules'. Três estágios: (1) codificação de estado em texto, (2) LLM gera candidatos via prompting, (3) ILP refina/otimiza. O loop autônomo que dirige scheduling é ILP + heuristics clássicos; LLM não é o agente agentic que percebe, raciocina, age. Seção IV methodology (A-E) detalha ILP como refinement obrigatório, não LLM como decisor.
- **agrees_with_regex:** True
